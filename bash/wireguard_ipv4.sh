...
read -p "Do you want to generate a config file for the client VPN? (y/n): " 

if [ $CONFIG_FILE_Y/N == "y" ]; then
    read -p "CONFIG_FILE/QR_CODE " CONFIG_FILE_EXPORT
    IP_PUBLIC=$(curl -4 ifconfig.net)

    mkdir ~/.wireguard-keys/wireguard-client/
    wg genkey | tee ~/.wireguard-keys/wireguard-client/privatekey | wg pubkey > ~/.wireguard-keys/wireguard-client/publickey
    ...
    read -p "Enter the range fot the client VPN: EX:10.8.0.2, 10.8.0.100" RANGE
    wg set wg0 peer $PUBLIC_KEY_CLIENT allowed-ips $RANGE

    if [[ $CONFIG_FILE_EXPORT == "CONFIG_FILE" ]]; then (
        echo "Archivo creado en ~/.wireguard-keys/wireguard-client/wg0-client.conf"
    elif [ $CONFIG_FILE_EXPORT == "QR_CODE" ]; then
        qrencode -t ansiutf8 < ~/.wireguard-keys/wireguard-client/wg0-client.conf
    fi
fi
